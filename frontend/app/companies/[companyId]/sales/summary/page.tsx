...
export default function SalesSummaryPage() {
  ...
  // Sync state if settings change (handled by Context mostly, but we re-init dates on mode change)
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    ...
    return (invoices as any[]).filter((inv) => {
      if (effectiveFromAD && inv.date < effectiveFromAD) return false;
      if (effectiveToAD && inv.date > effectiveToAD) return false;
      if (customerId && String(inv.customer_id) !== customerId) return false;

      if (voucherNumberFilter.trim()) {
        const vNum = (inv.voucher_number || "").toString().toLowerCase();
        if (!vNum.includes(voucherNumberFilter.trim().toLowerCase())) return false;
      }

      if (billDateFilter.trim()) {
        const bDate = inv.bill_date || "";
        const bDateBS = bDate ? (safeADToBS(bDate) || bDate) : "";
        const search = billDateFilter.trim().toLowerCase();
        if (!bDate.toLowerCase().includes(search) && !bDateBS.toLowerCase().includes(search)) return false;
      }
  ...
  }, [invoices, fromDate, toDate, customerId, itemNameFilter, items, isBS]);
  ...
  const detailedRows = useMemo(() => {
    ...
    for (const inv of filtered) {
      if (!inv.lines || !Array.isArray(inv.lines)) continue;
      for (const line of inv.lines) {
        ...
        const customerName = customer?.name || `Customer #${inv.customer_id}`;

        const warehouse = warehouses?.find((w: any) => w.id === line.warehouse_id);
        const warehouseName = warehouse?.name || "";

        const spId = line.sales_person_id || inv.sales_person_id;
        const sp = salesPersons?.find((s: any) => s.id === spId);
        const spName = sp?.name || "";

        const depId = line.department_id || inv.department_id;
        const dep = departments?.find((d: any) => d.id === depId);
        const depName = dep?.name || "";

        const projId = line.project_id || inv.project_id;
        const proj = projects?.find((p: any) => p.id === projId);
        const projName = proj?.name || "";

        const segId = line.segment_id || inv.segment_id;
        ...
        // TDS is stored at the invoice level; distribute proportionally across lines
        const lineProportion = inv.lines.length > 1 ? 1 / inv.lines.length : 1;
        const invTdsAmount = Number(inv.tds_amount || 0);
        const lineTdsAmount = inv.apply_tds ? invTdsAmount * lineProportion : 0;

        rows.push({
          date: isBS ? (safeADToBS(inv.date) || inv.date) : inv.date,
          bill_date: inv.bill_date ? (isBS ? (safeADToBS(inv.bill_date) || inv.bill_date) : inv.bill_date) : "",
          voucher_number: inv.voucher_number || "",
          due_date: isBS ? (safeADToBS(inv.due_date || inv.date) || inv.due_date || inv.date) : (inv.due_date || inv.date),
          bill_no: inv.reference,
          custom_reference: inv.custom_reference || "",
          ...
          payment_mode: paymentModes && Array.isArray(paymentModes)
            ? paymentModes.find((p: any) => p.id === inv.payment_mode_id)?.name || ""
            : "",
    ...
    return rows;
  }, [filtered, items, customers, warehouses, salesPersons, departments, projects, segments, paymentModes, isBS]);
  ...
  const summaryRows = useMemo(() => {
    ...
    filtered.forEach((inv) => {
      if (inv.lines && Array.isArray(inv.lines)) {
        inv.lines.forEach((l: any) => {
          ...
          const customerName = customers?.find((c: any) => c.id === inv.customer_id)?.name || `Customer #${inv.customer_id}`;
          ...
          const base = qty * rate - disc;
          const tax = base * (taxRate / 100);
          const linesCount = inv.lines?.length || 1;
          const invTds = Number(inv.tds_amount || 0);
          const lineTds = inv.apply_tds ? invTds / linesCount : 0;
          ...
          g.customer_name.add(customerName);
        });
      }
    });
    ...
    return Array.from(groupedMap.values()).map((g: any) => ({
      ...g,
      customer_name: Array.from(g.customer_name).join(", "),
      hs_code: Array.from(g.hs_code).join(", "),
    }));
  }, [filtered, customers, items, salesPersons, departments, projects, segments, paymentModes, isBS]);

  const grandTotal = useMemo(
    () => filtered.reduce((sum, inv) => sum + invoiceTotal(inv), 0),
    [filtered]
  );

  const handleExportCsv = () => {
    if (!filtered.length) return;
    const headers = ["Date", "Customer", "Reference", "Total"];
    const rows = filtered.map((inv: any) => {
      const customerName =
        customers?.find((c: any) => c.id === inv.customer_id)?.name ||
        `Customer #${inv.customer_id}`;
      return [
        isBS ? (safeADToBS(inv.date) || (inv.date ?? "")) : (inv.date ?? ""),
        customerName,
        inv.reference ?? "",
    ...
    a.href = url;
    a.download = `sales-summary-${companyId}.csv`;
    a.click();
  ...
  const handleExportDetailedCsv = () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `sales-detailed-${companyId}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };
  ...
  const handleExportExcelDetailed = () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `sales-detailed-${companyId}.xls`;
    a.click();
    URL.revokeObjectURL(url);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 shadow-sm overflow-hidden select-none">
        <div className="h-[3px] w-full bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 opacity-60" />
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between px-5 py-3">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-base font-bold text-slate-800 dark:text-slate-100 tracking-tight">Sales Register</h1>
              <p className="text-xs text-slate-500 dark:text-slate-400">Analyze your sales performance and transaction details</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              <svg className="w-4 h-4 text-slate-400 group-hover:text-indigo-500 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div className="relative z-[60] rounded-xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 shadow-sm p-4 mt-1 flex flex-wrap gap-4 items-end">
        <div className="flex flex-col gap-1.5">
          <label className="block text-xs font-black uppercase tracking-wider text-slate-400">Calendar Mode</label>
          <select
            className="h-9 border border-indigo-500/20 rounded-xl px-3 text-xs bg-white dark:bg-slate-950 shadow-sm focus:ring-4 focus:ring-indigo-500/10 outline-none font-bold text-indigo-700 border-t-2 border-t-indigo-500 min-w-[150px]"
        ...
        <div className="flex flex-col gap-1.5 min-w-[150px]">
          <label className="block text-xs font-black uppercase tracking-wider text-slate-400">Customer</label>
          <select
        ...
        <div className="flex-1 min-w-[200px]">
          ...
          <input
            type="text"
            className="h-9 w-full border border-slate-200 dark:border-slate-700 rounded-xl text-xs px-4 bg-white dark:bg-slate-950 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none italic"
        ...
        <div className="flex gap-2 items-center ml-auto">
          ...
          <button
            type="button"
            className="h-9 px-6 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs shadow-lg active:scale-95 transition-all disabled:opacity-50 uppercase tracking-widest"
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 shadow-sm px-6 py-4 flex flex-col gap-4 animate-in fade-in slide-in-from-top-1 duration-500">
        <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-6">
                ...
                <div className="px-4 py-1 rounded-full bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800 text-xs font-black text-indigo-700 dark:text-indigo-300 shadow-inner">
                   Total Revenue: {new Intl.NumberFormat('en-US', { minimumFractionDigits: 2 }).format(grandTotal)}
                </div>
            </div>
        </div>
          ...
          {AVAILABLE_COLUMNS.map(col => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-[11px] font-bold border transition-all select-none shadow-sm ${visibleCols[col.id]
                  ...
                  : "bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-indigo-400 hover:text-indigo-600"
                }`}
            >
              {col.label}
            </button>
        ...
        <div className="flex items-center justify-between mb-8 border-b border-slate-100 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-black text-slate-900 dark:text-slate-100 tracking-tighter uppercase italic">
              {reportType === "detailed" ? "Full Disclosure Report" : "Consolidated Summary"}
            </h2>
          </div>
          <div className="flex bg-slate-100/60 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700 shadow-inner">
            <button
              ...
              className={`px-6 py-2 rounded-lg text-xs font-black transition-all uppercase tracking-widest ${reportType === "summary" ? "bg-white dark:bg-slate-700 shadow-md text-indigo-600 dark:text-indigo-300" : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"}`}
            >
              Summary
            </button>
            <button
              onClick={() => setReportType("detailed")}
              className={`px-6 py-2 rounded-lg text-xs font-black transition-all uppercase tracking-widest ${reportType === "detailed" ? "bg-white dark:bg-slate-700 shadow-md text-indigo-600 dark:text-indigo-300" : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"}`}
            >
              Detailed
            </button>
          </div>
              ...
              <table className="w-full text-xs whitespace-nowrap">
                ...
                <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-100 dark:divide-slate-800/60">
                  {summaryRows.map((row, idx) => (
                    <tr key={idx} className="transition-all hover:bg-indigo-50/40 dark:hover:bg-indigo-900/10 group">
                      {columnOrder.map(colId => {
                        const col = AVAILABLE_COLUMNS.find(c => c.id === colId);
                        if (!col) return null;
                        ...
                        return (
                          <td key={col.id} className={`py-3.5 px-4 text-slate-700 dark:text-slate-300 font-medium ${isNumber ? "text-right tabular-nums text-slate-900 dark:text-slate-100 font-black" : "group-hover:text-indigo-600 group-hover:dark:text-indigo-400"}`}>
                  ...
                  <tr>
                    {columnOrder.map((colId, idx) => {
                      ...
                      if (col.id === "amount") {
                        ...
                        return <td key={col.id} className="py-4 px-4 text-right tabular-nums text-indigo-700 dark:text-indigo-400 border-l border-slate-100 dark:border-slate-700/50" title="Total Amount">{totalAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>;
                      }
                      if (col.id === "quantity") {
                        const totalQty = summaryRows.reduce((sum, r) => sum + (r.quantity || 0), 0);
                        return <td key={col.id} className="py-4 px-4 text-right tabular-nums text-indigo-700 dark:text-indigo-400 border-l border-slate-100 dark:border-slate-700/50" title="Total Quantity">{totalQty.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>;
                      }
                      if (col.id === "tax_amount") {
                        const totalTax = summaryRows.reduce((sum, r) => sum + (r.tax_amount || 0), 0);
                        return <td key={col.id} className="py-4 px-4 text-right tabular-nums text-indigo-700 dark:text-indigo-400 border-l border-slate-100 dark:border-slate-700/50" title="Total Tax">{totalTax.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>;
                      }
                      if (col.id === "discount") {
                        const totalDisc = summaryRows.reduce((sum, r) => sum + (r.discount || 0), 0);
                        return <td key={col.id} className="py-4 px-4 text-right tabular-nums text-indigo-700 dark:text-indigo-400 border-l border-slate-100 dark:border-slate-700/50" title="Total Discount">{totalDisc.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>;
                      }
                      return <td key={col.id} className={`py-4 px-4 font-black text-[9px] uppercase tracking-widest text-slate-400 ${idx === 0 ? "" : "border-l border-slate-100 dark:border-slate-700/50"}`}>{idx === 0 ? "Consolidated Totals" : ""}</td>;
                    })}
                  </tr>
                </tfoot>
              </table>
               ...
               <table className="w-full text-xs whitespace-nowrap">
                  ...
                  <tbody className="bg-white dark:bg-slate-900">
                    {detailedRows.map((row, idx) => (
                      <tr key={idx} className="transition-all hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 border-b border-slate-50 dark:border-slate-800/40 last:border-0 group">
                        {columnOrder.map(colId => {
                          const col = AVAILABLE_COLUMNS.find(c => c.id === colId);
                          if (!col) return null;
                          ...
                          const isNumber = typeof val === "number";
                          return (
                            <td key={col.id} className={`py-3 px-4 text-slate-600 dark:text-slate-400 font-medium ${isNumber ? "text-right tabular-nums font-black text-slate-900 dark:text-slate-100" : "group-hover:text-indigo-600 transition-colors"}`}>
                              {isNumber ? Number(val).toLocaleString(undefined, { minimumFractionDigits: 2 }) : val}
                            </td>
                          );
                        })}
                  ...
                  <tfoot className="bg-slate-800 text-white font-black z-10 shadow-2xl relative">
                    <tr className="border-t border-slate-700">
                      {columnOrder.map((colId, idx) => {
                        ...
                        return <td key={col.id} className={`py-4 px-4 font-black uppercase text-[9px] tracking-[0.2em] italic ${idx === 0 ? "text-indigo-400" : "border-l border-slate-700/50 text-transparent"}`}>
                          {idx === 0 ? "Report Consolidated Totals" : ""}
                        </td>;
                      })}
                    </tr>
                  </tfoot>
