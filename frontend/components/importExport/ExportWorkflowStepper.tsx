...
export type ExportWorkflowStepKey = (typeof STEPS)[number]["key"];

export function ExportWorkflowStepper({ activeKey }: { activeKey: ExportWorkflowStepKey }) {
  const idx = STEPS.findIndex((s) => s.key === activeKey);
  return (
    <nav aria-label="Export workflow" className="rounded-lg border border-slate-200 bg-slate-50/80 p-2 dark:border-slate-700 dark:bg-slate-900/50">
