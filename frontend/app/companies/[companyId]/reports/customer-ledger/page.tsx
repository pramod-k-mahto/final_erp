  ...
  const handleView = async () => {
    ...
    try {
      const res = await api.get<CustomerLedgerResponse>(
        `/companies/${companyId}/reports/customer-ledger`,
        {
          params: {
      ...
      setLedgerData(null);
      setError(
        typeof detail === "string" ? detail : "Failed to load customer ledger."
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between px-4 py-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-bold text-slate-800 dark:text-slate-100 tracking-tight">Customer Ledger Report</h1>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-tight">Customer account transaction history</p>
            </div>
          </div>
      ...
      >
        <div className="px-5 py-3 flex items-center gap-2 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950">
          <span className="text-slate-800 dark:text-slate-200 text-sm font-semibold tracking-wide">🔍 Customer & Date Filters</span>
        </div>
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 shadow-sm p-4 text-sm space-y-3">
        {ledgerData ? (
          <div ref={printRef}>
            ...
            >
              <div className="mb-2">
                ...
                >
                  Customer Ledger: {ledgerData.ledger_name}
                </div>
                <div
                  style={{
                    display: "flex",
                ...
                >
                  <span>Account: {ledgerData.customer_name || selectedCustomerName}</span>
                  <span>{`Linked ledger: ${ledgerData.ledger_name}`}</span>
                </div>
                <div
                  style={{
                    display: "flex",
                ...
                <tbody>
                  ...
                  {ledgerData.transactions.map((t) => (
                    ...
                    >
                      ...
                      <td className="py-1 px-1 text-xs">{mapVoucherType(t.voucher_type)}</td>
                      <td className="py-1 px-1 text-xs text-slate-600">
                      ...
                      <td className="py-1 px-1 text-right" />
                      <td className="py-1 px-1 text-right">{formatMoney(t.debit)}</td>
                      <td className="py-1 px-1 text-right">{formatMoney(t.credit)}</td>
                      <td className="py-1 px-1 text-right">
                        {(() => {
                          const isDoc =
                            t.voucher_type === "SALES_INVOICE" ||
                            t.voucher_type === "PURCHASE_BILL";
                  ...
                  <tr className="border-t">
                    ...
                    <td className="py-1 px-1" />
                    <td className="py-1 px-1" />
                    <td className="py-1 px-1 text-right font-medium">Operation Total:</td>
                    <td className="py-1 px-1 text-right" />
                    <td className="py-1 px-1 text-right font-medium">{formatMoney(totalDebit)}</td>
                    <td className="py-1 px-1 text-right font-medium">{formatMoney(totalCredit)}</td>
                    <td className="py-1 px-1 text-right" />
                    <td className="py-1 px-1 text-right" />
                  </tr>
                  <tr className="border-t">
                    ...
                    <td className="py-1 px-1" />
                    <td className="py-1 px-1" />
                    <td className="py-1 px-1 text-right font-medium">Closing Balance :</td>
                    <td className="py-1 px-1 text-right" />
                    <td className="py-1 px-1 text-right" />
          ...
          <div className="text-xs text-slate-500">
            Please select a customer and date range, then click &quot;View Ledger&quot;.
          </div>
        )}
      </div>
    </div>
