...
import { api, PurchaseReturn, getCurrentCompany } from "@/lib/api";
...
export default function PurchaseReturnDetailPage() {
  ...
  const { data: ret } = useSWR<PurchaseReturn | null>(
    companyId && returnId ? `/purchases/companies/${companyId}/returns/${returnId}` : null,
    fetcher
  );
  ...
  const totals = useMemo(() => {
    if (!ret?.lines) return { subtotal: 0, taxTotal: 0, discountTotal: 0, grandTotal: 0 };
    let subtotal = 0;
    let taxTotal = 0;
    let discountTotal = 0;
  ...
  if (!ret) {
    return <div className="text-sm text-slate-500">Loading purchase return...</div>;
  }
      ...
      <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 shadow-sm overflow-hidden mb-6 no-print">
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-800 dark:text-slate-100 tracking-tight">Purchase Return #{ret.id}</h1>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-tight mt-0.5">
                Date: {ret.date} · Supplier: {ret.supplier_name || (ret.supplier_id ? `#${ret.supplier_id}` : "")}
              </p>
            </div>
          </div>
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 shadow-sm p-4 text-sm space-y-4">
        ...
        <div className="mt-3 flex flex-col gap-2 border-b border-slate-200 pb-2 text-xs text-slate-700 md:flex-row md:items-start md:justify-between">
          <div className="space-y-0.5">
            ...
            <div>Supplier: {ret.supplier_name || (ret.supplier_id ? `#${ret.supplier_id}` : "")}</div>
          </div>
            ...
            <tfoot className="border-t border-slate-200 bg-slate-50/50">
              ...
              {totals.tdsAmount > 0 && (
                <tr className="border-t border-slate-300 bg-emerald-50">
                  <td colSpan={7} className="py-2 px-2 text-right font-black text-emerald-800 uppercase tracking-widest text-[10px]">Net Payable:</td>
